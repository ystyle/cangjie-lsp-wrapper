...
崩溃感知前的"微窗口"（子进程已死但 wrapper 尚未读到 EOF，通常仅几毫秒）内写入的 client 消息可能落进无人读取的管道。为把丢失降到最低，**任何向子进程写失败的消息**（EPIPE/写错误）自动触发失败流程：request 先入缓冲队列（重启完成后原样重发，响应自然回到 client），再按连续失败计数走重启或冷却；若计数超限进入冷却，缓冲队列中的 request 逐条回显 `-32603` 错误而非无声丢弃。

**会话事件幂等**：每个 server 会话的 stdout reader goroutine 与 `cmd.Wait` 收尾 goroutine 都会观察到进程结束，若两者各发一次 EOF 事件会把重启/冷却流程重复触发两次。因此 EOF 事件只由 reader goroutine 发出（Wait 仅收集退出码记日志），supervisor 端再按会话序号对 EOF 做幂等去重，双保险。
...
**参数比例约束（重要）**：健康清零的稳定期（`STABLE_RESET_SECS`）必须**明显大于**单轮"探活周期 + 探活超时 + 一次重启耗时"之和。若稳定期过短，探活-重启循环中每次短暂的稳定运行都会触发清零、把失败计数吞掉，导致连续失败永远达不到熔断阈值（表现为"为什么一直重启却从不进冷却"）。默认值 20s 探活周期 / 10s 超时 / 120s 稳定期即按此比例设定；自行调参时保持稳定期 ≥ 数倍轮询周期。
